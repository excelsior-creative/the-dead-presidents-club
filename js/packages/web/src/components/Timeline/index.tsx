...
const launchDate = new Date(2021, 9, 1);

export const Timeline = () => {
  return (
    <div className="dpc-timeline">
      <h2>Release Schedule</h2>
      <p>
        The following release schedule is planned to allow ample for planning
        and distribution. Be aware that for some Presidents we truly do not know
        when they will be released.
      </p>
      <Steps direction="vertical">
        {presidents.map((p: any, i: number) => {
          const dt = new Date(launchDate.getTime() + 7 * 86400000 * i);
          // todo - calculate the current one based on the date

          const numberEditions = p.took_office.substring(0, 4);

          const launchString =
            `${numberEditions} editions launching: ` +
            (p.death_year ? dt.toLocaleDateString() : '???');

          return <Step title={p.president} description={launchString} />;
        })}
      </Steps>
    </div>
  );
